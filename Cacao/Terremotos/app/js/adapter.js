...
Adapter = (function(){
	...
	var insertarTerremotos = function(){
		...
		for (var i =0; i<len; i++){
			...
			pushTerremotosDB(element);
		};
	}
	
	Adapter.prototype.crearDB = function(){
		webSql.init(insertarTerremotos);
	}
	...
	var mostrarTerremotoMapa = function(terremotos){
		var coords = [];
		for (var i=0;i<terremotos.length;i++){
			coords.push(terremotos[i].location.split(',')); 
		}
	...
	var getTerremotoDBById = function(id){
		webSql.getTerremotoById(id,mostrarTerremoto);
	}
	
	var pushTerremotosDB = function(terremotoWeb){
		getTerremotoDBById(terremotoWeb.id);
	}
